...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # so we can get dataset
        print("DATASET IS: ")
        print(self.val_dataloader.dataset)
    ...
    def get_second_rollout_batch(self, og_output_gen_batch, original_full_batch, append=False, save_images=False):
        ...
        print(second_rollout_batch_dict.keys())
        ...
        print("BATCH KEYS")
        print(original_full_batch.batch.keys())
        print("NON TENSOR KEYS")
        print(original_full_batch.non_tensor_batch.keys())
        ...
        if not append:
            ...
            for key in original_full_batch.batch.keys():
                ...
                if key in keys_to_merge:
                    print(f"CONCATING {key}")
                    print(f"{original_full_batch.batch[key][0].shape} - {second_rollout_batch.batch[key][0].shape}")

                    print(type(original_full_batch.batch[key][0]))
                    for index, og_index in enumerate(tool_use_indices):
                        print(f"{original_full_batch.batch[key][og_index].shape} - {second_rollout_batch.batch[key][index].shape}")
                        original_full_batch.batch[key][og_index] = torch.cat((original_full_batch.batch[key][og_index], second_rollout_batch.batch[key][index]))
                    continue
            ...
            print(f"ORIGINAL LENGTH: {len(original_full_batch.batch[first_key])}")
            ...
            #config.worker.actor.micro_batch_size_per_device_for_experience

            print(f"PADDING SIZE: {append_padding_size}")
            
            merged_data = {}
            for key in second_rollout_batch.batch.keys():
                if key not in original_full_batch.batch:
                    print(f"KEY NOT FOUND - B: {key}")
                    continue
            ...
            print(f"APPENDED LENGTH: {len(original_full_batch.batch[first_key])}")
    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                #dict_keys(['figure_id', 'figure_path', 'annotation_path', 'table_path', 'query', 'prompt', 'multi_modal_data', 'multi_modal_inputs', 'metadata', 'raw_prompt_ids', 'ground_truth'])
                ...
                with timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == "remax":
                        print("this should not get executed!!! not REMAX")
                    ...
                    batch, tool_stats = self.get_second_rollout_batch(gen_batch_output, batch, append=self.config.worker.reward.double_reward)
